...
def _build_cards_delta_hint(*, current_cards: list[Any], previous_cards: list[Any]) -> str:
    ...
    if overlap:
        ...
        if changed_orientation:
            ...
            return (
                "Факт из истории: уже встречалась та же карта, но ориентация изменилась "
                f"({cards_str}). Обязательно коротко подсвети, как это меняет динамику ситуации."
            )
        if stable_overlap:
            cards_str = ", ".join(f"«{_display_card_name(n)}»" for n in stable_overlap[:2])
            return (
                f"Факт из истории: снова выходит та же карта ({cards_str}). "
                "Обязательно мягко отметь повтор линии и сравни, что в ситуации осталось тем же, а что сдвинулось."
            )

    suit = str(snapshot.get("common_suit") or "")
    if suit:
        return (
            f"Факт из истории: в обоих раскладах повторяется акцент на масти «{suit}». "
            "Обязательно подсвети схожую линию и коротко сравни изменения."
        )

    motif = str(snapshot.get("common_motif") or "")
    if motif:
        return (
            f"Факт из истории: повторяется смысловая линия «{motif}». "
            "Мягко подчеркни сходство и сравни текущую динамику с прошлым раскладом."
        )
